      ...
      <section className="pt-32 pb-20 relative overflow-hidden">
        ...
        <div className="container-main relative">
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-bold tracking-[-0.03em] text-white mb-6 leading-[1.05]">
            ...
            <p className="text-lg md:text-xl text-white/50 leading-relaxed mb-8 max-w-2xl font-medium">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {layers.map((layer, index) => (
              <motion.div
                ...
                className="group bg-white/[0.02] border border-white/[0.06] backdrop-blur-xl rounded-2xl p-8 hover:border-brand-primary/30 hover:bg-white/[0.03] hover:shadow-[0_0_30px_rgba(26,109,214,0.05)] transition-all duration-300 hover:scale-[1.01]"
              >
                <div className="flex justify-between items-start mb-6">
                  <div className="w-12 h-12 bg-white/[0.03] flex items-center justify-center text-white/40 rounded-xl group-hover:bg-brand-primary/10 group-hover:text-brand-primary group-hover:border group-hover:border-brand-primary/20 transition-all duration-300">
                    {layer.icon}
                  </div>
                  <span className="text-xs text-white/20 font-bold uppercase tracking-widest">{layer.id}</span>
                </div>

                <h3 className="text-xl font-bold tracking-[-0.015em] text-white mb-3 group-hover:text-brand-primary transition-colors">
                  {layer.title}
                </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div
                ...
                className="bg-white/[0.02] border border-white/[0.06] backdrop-blur-xl rounded-2xl p-6 hover:border-brand-primary/20 hover:bg-white/[0.03] hover:shadow-[0_0_20px_rgba(26,109,214,0.02)] transition-all duration-300 group hover:scale-[1.01]"
              >
                <span className="text-xs text-brand-primary/60 font-bold block mb-4 uppercase tracking-widest">Node {node.tag}</span>
                <h4 className="text-lg font-bold tracking-[-0.015em] text-white mb-3 group-hover:text-brand-primary transition-colors">{node.label}</h4>
                <p className="text-sm leading-relaxed text-white/50 font-medium">{node.text}</p>
              </motion.div>
            ))}
          </div>
        </div>
