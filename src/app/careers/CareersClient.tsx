...
export default function CareersClient() {
  return (
    <main id="main-content" aria-label="Careers at Maysan Labs" className="min-h-screen bg-[var(--bg-dark)] relative overflow-hidden flex flex-col">
      ...
      <section className="py-16 border-y border-white/[0.05] relative">
            ...
            {[
              ...
              >
                <div className="bg-white/[0.02] border border-white/[0.05] backdrop-blur-md rounded-2xl p-6 md:p-8 transition-all duration-300 hover:border-white/[0.1] hover:-translate-y-1 h-full">
                  ...
                  <h3 className="text-lg font-semibold text-gray-50 mb-3 group-hover:text-blue-400 transition-colors tracking-tight">{value.title}</h3>
      ...
      <section id="openings" className="py-20">
        <div className="container-main">
            ...
            {jobPositions.map((job, index) => (
              ...
              >
                <Link href={`/careers/apply?role=${job.id}`} className="block h-full">
                  <div className="bg-white/[0.02] border border-white/[0.05] backdrop-blur-md rounded-2xl p-6 md:p-8 transition-all duration-300 hover:border-white/[0.1] hover:-translate-y-1 h-full flex flex-col">
                    <div className="flex items-center justify-between mb-5">
                      <span className="text-[11px] font-semibold px-3 py-1 rounded-full bg-blue-500/10 text-blue-400 uppercase tracking-wide">
                        {job.category}
                      </span>
                    ...
                    <h3 className="text-xl md:text-2xl font-bold text-gray-50 mb-3 group-hover:text-blue-400 transition-colors tracking-tight">
                      {job.title}
                    </h3>
                    <p className="text-gray-400 text-sm leading-relaxed mb-6 flex-1">
                      {job.description}
                    </p>
                    <div className="flex items-center gap-2 pt-4 border-t border-white/[0.06] mb-5">
                      <MapPin size={14} className="text-blue-400 shrink-0" />
                      <span className="text-sm text-gray-400">{job.location}</span>
                    </div>
                    <div className="flex items-center gap-2 text-blue-400 text-sm font-semibold group-hover:gap-3 transition-all mt-auto">
          ...
          <div className="mt-16 bg-white/[0.02] border border-white/[0.05] backdrop-blur-md rounded-2xl p-8 md:p-12 text-center">
            <h3 className="text-2xl md:text-3xl font-semibold text-gray-50 mb-4">Don&apos;t see a perfect fit?</h3>
