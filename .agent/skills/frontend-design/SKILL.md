...
## 🏆 MAYSAN LABS OFFICIAL DESIGN SYSTEM REFERENCE (P0 - MANDATORY)

> [!IMPORTANT]
> **All frontend and visual edits inside the Maysan Labs workspace MUST strictly adhere to this design reference system to preserve visual unity.**

### 1. Unified Typography Rules
*   **Base Font Family:** **`Outfit`** (configured as the default `@apply font-sans;` globally).
*   **High-End Geometric Display Headings:**
    *   `text-xl md:text-3xl font-bold tracking-[-0.015em] leading-tight` (Sub-cards/Section titles)
    *   `text-3xl md:text-5xl font-bold tracking-[-0.02em] leading-tight` (Display sections)
    *   `text-4xl md:text-7xl font-bold tracking-[-0.03em] leading-[1.05]` (Hero main folds)
*   **Contrast Rules:** Always use high-contrast parameters (e.g. `text-white/60 font-medium` instead of generic low-contrast grey tags) to maintain legibility on all viewports.
*   **Display Text Gradients:** Apply analogous linear gradients (`bg-gradient-to-r from-[#1A6DD6] to-[#00d2ff] bg-clip-text text-transparent`) paired with subtle glowing dropshadow filter properties.

### 2. Corporate & Product Color Palettes
*   **Corporate Base Canvas:** Ultra-deep space blue-black (`#03050d` to `#000000`) with ambient cosmic light blurs (`blur-[120px]`) and integrated wireframe grids.
*   **Primary Corporate Accent:** Cyber Royal Blue (`#1A6DD6`) paired with Cyan (`#00d2ff`).
*   **Educational / Dashboard Accent:** Glowing Emerald (`#10b981`) paired with Cyber Teal (`#14b8a6`).
*   **Translucent Borders:** Translucent glassmorphic borders (`border-white/[0.06] bg-white/[0.02] backdrop-blur-xl`).

### 3. Glassmorphic Glare Action Buttons
Always build visual buttons utilizing modern glare layers and ambient drop shadows:
*   **Corporate Glass Button:** `bg-gradient-to-r from-[#1A6DD6] to-[#00d2ff] text-white hover:shadow-[0_0_30px_rgba(26,109,214,0.3)] shadow-[0_0_15px_rgba(26,109,214,0.15)] transition-all duration-300 hover:scale-[1.02]`
*   **Dashboard / SMS Action Button:** `bg-gradient-to-r from-[#10b981] to-[#14b8a6] text-black hover:shadow-[0_0_30px_rgba(16,185,129,0.35)] shadow-[0_0_15px_rgba(16,185,129,0.15)] transition-all duration-300 hover:scale-[1.02] font-bold uppercase tracking-wider`

### 4. Widescreen Showcase Banners (21:9 ratio)
*   **Viewport Ratio:** Always enclose widescreen mockups in custom `aspect-[21/9] w-full overflow-hidden relative` layouts.
*   **Edge Feathering Masks:** Center square logos and blend them using horizontal opacity fading gradients. Avoid hard pixel boundaries at all costs.

### 5. Version-3 Cache-Busting Packaging
*   **PWA Assets:** Always request the Version 3 cached files: `/icon-192x192-v3.png` and `/icon-512x512-v3.png`.
*   **Header Policies:** Configure Next.js server overrides in `next.config.ts` to enforce `no-store, no-cache` bypass policies for all manifest and logo updates.
...
| **Technical Visual Clutter / Hacker UI** | Looks highly unprofessional, amateurish, and weird for premium software/design studios. Avoid command terminal logs, telemetry nodes, active latency counts, and neon diagnostic visuals. | Clean, editorial layouts with premium typography, massive breathing room, and minimalist abstract vector blocks. |
